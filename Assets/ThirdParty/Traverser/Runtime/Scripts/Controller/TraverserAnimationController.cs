...
namespace Traverser
{
    ...
    public class TraverserAnimationController : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            CaptureEffector(hipsRigEffector, ref hipsEffectorOriginalTransform);
            CaptureEffector(spineRigEffector, ref spineEffectorOriginalTransform);
            CaptureEffector(leftLegRigEffector, ref leftLegEffectorOriginalTransform);
            CaptureEffector(rightLegRigEffector, ref rightLegEffectorOriginalTransform);
            CaptureEffector(aimRigEffector, ref aimEffectorOriginalTransform);

            if (animator != null)
            {
                foreach (AnimatorControllerParameter param in animator.parameters)
                {
                    if (!animatorParameters.ContainsKey(param.name))
                        animatorParameters.Add(param.name, param.nameHash);
                }
            }
        }

        static void CaptureEffector(GameObject effector, ref TraverserTransform stored)
        {
            if (effector == null)
            {
                stored = TraverserTransform.Get(Vector3.zero, Quaternion.identity);
                return;
            }

            stored = TraverserTransform.Get(effector.transform.localPosition, effector.transform.localRotation);
        }
        ...
        {
            skeletonPosition = BonePosition(HumanBodyBones.Hips, transform.position + Vector3.up * 0.9f);
            rightFootPosition = BonePosition(HumanBodyBones.RightFoot, transform.position);
            leftFootPosition = BonePosition(HumanBodyBones.LeftFoot, transform.position);
            rightHandPosition = BonePosition(HumanBodyBones.RightHand, transform.position + Vector3.up * 1.2f);
            leftHandPosition = BonePosition(HumanBodyBones.LeftHand, transform.position + Vector3.up * 1.2f);
        }

        Vector3 BonePosition(HumanBodyBones bone, Vector3 fallback)
        {
