        ...
        TraverserAbility SimulatePrediction(ref TraverserCharacterController controller, float deltaTime)
        {
            ...
            for (int i = 0; i < iterations; ++i)
            {
                    ...
                    || controller.CheckForwardCollision(transform.position - Vector3.up * 0.25f, contactDistanceMax))
                {
                    ...
                    || (collision.ground && controller.previous.ground && collision.ground != controller.previous.ground)) // we are dropping/falling down and found ground
                {
                    ...
                    && (controller.previous.ground == null || distanceToGround < 0.15f) )
                {
                        ...
                        && distanceToGround < controller.capsuleHeight * 1.25f)
                    {
                        ...
                        TraverserTransform targetTransform = TraverserTransform.Get(TraverserCharacterController.SafeClosestPoint(collision.ground, animationController.skeletonPos)
